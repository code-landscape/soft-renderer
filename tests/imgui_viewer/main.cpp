...
int main(int, char **) {
  ...
  style.WindowRounding = 8.0f;    // Rounded windows
  style.ChildRounding = 8.0f;     // Rounded child windows
  style.PopupRounding = 8.0f;     // Rounded popups/tooltips
  style.FrameRounding = 4.0f;     // Rounded buttons, inputs, checkboxes, etc.
  style.GrabRounding = 4.0f;      // Rounded slider/scrollbar grabs
  style.ScrollbarRounding = 9.0f; // Already rounded by default
  style.TabRounding = 5.0f;       // Already rounded by default
  ...
  ImFont *font = io.Fonts->AddFontFromFileTTF(
      "/usr/share/fonts/TTF/JetBrainsMonoNerdFont-Regular.ttf", 8);
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    {
      ImGui::Begin("Hello textures!");
      ImGui::Text("Learning vulkan to upload an image already...");
      ImGui::Text("kinda hard to handle vulkan TwT");
      ImGui::End();
    }
